...
class Post
{
  ...
  public function create()
  {
    ...
    if (empty(trim($this->title)) || empty(trim($this->author)) || empty(trim($this->content))) {
      header("Location: http://localhost/task02/post/create_post.php");
      return;
    }
    $db->create($this->title, $this->author, $this->content);
    header('Location: http://localhost/task02/post/list_posts.php');
  }
  ...
  public function update($id)
  {
    $db = new Database();
    if (empty(trim($this->title)) || empty(trim($this->author)) || empty(trim($this->content))) {
      header("Location: http://localhost/task02/post/edit_post.php?id=$id");
      return;
    }
    $db->update($id, $this->title, $this->author, $this->content);
    header("Location: http://localhost/task02/post/list_posts.php");
  }
